...
def evaluate(dataset, predictions):
    f1 = exact_match = total = p = r = 0
    for article in dataset:
        for paragraph in article['paragraphs']:
            for qa in paragraph['qas']:
                total += 1
                q = unicode(str(qa['id']), "utf-8")
...
def evaluate(dataset, predictions):
    f1 = exact_match = p_total = r_total = p = r = 0
    for article in dataset:
        for paragraph in article['paragraphs']:
            for qa in paragraph['qas']:
                q = unicode(str(qa['id']), "utf-8")
                ...
                if ground_truths[0] != 'NIL':
                    r_total += 1
                if prediction != 'NIL':
                    p_total += 1
                # if ground_truths[0] != 'NIL':
                    # print("g: ", ground_truths[0], "p: ", prediction)
                if prediction != 'NIL':
                    exact_match += metric_max_over_ground_truths(
                        exact_match_score, prediction, ground_truths)
                    p += metric_max_over_ground_truths(
                        ...
                        rec_score, prediction, ground_truths)
                # print(p, r, 2*p*r/(p+r))
    exact_match = 100.0 * exact_match / r_total
    p = 100.0 *	p / p_total
    r =	100.0 * r / r_total
    f1 = 2*p*r/(p+r)

    return {'exact_match': exact_match, 'f1': f1, 'recall': r, 'precision':p}
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Evaluation for NewsQA ' + expected_version)
