...
from namanda.reader import utils, vector, config, data
from namanda.reader import DocReader
from namanda import DATA_DIR as NAMANDA_DATA
...
def add_train_args(parser):
    ...
    runtime.add_argument('--batch-size', type=int, default=60,
    ...
    #general.add_argument('--finetune-partial-layers', type=int, default=0,
    #                     help='finetune subset of layers')
    #subsampling
    subsampling = parser.add_argument_group('Subsampling')
    subsampling.add_argument('--subsample', type='bool', default=False,
                         help='subsample training')
    subsampling.add_argument('--subsample-size', type=int, default=1000,
                         help='subsample training data to this number of examples')
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    if args.char_embedding:
        word_dict, char_dict = utils.build_word_and_char_dict(args, train_exs + dev_exs)
...
def main(args):
    ...
    if args.subsample:
        train_exs = random.sample(train_exs, int(args.subsample_size))
        logger.info('subsampled Num train examples = %d' % len(train_exs))
    ...
    if args.checkpoint and os.path.isfile(args.model_file + '.checkpoint'):
        ...
        model = DocReader.load_finetune(args.pretrained, args)
        ...
        # Set up partial tuning of layers
        #print(model)
