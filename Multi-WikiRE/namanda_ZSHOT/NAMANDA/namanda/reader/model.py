...
class DocReader(object):
    ...
    def load_embeddings(self, words, embedding_file):
        ...
        with open(embedding_file,encoding="utf8") as f:
            for line in f:
                ...
                #print(len(parsed), embedding.size(1) + 1)
                if len(parsed) == 2:
                   continue
    ...
    def init_optimizer(self, state_dict=None):
        ...
        #if self.args.finetune_partial_layers != 0:
        if True:
            #print("partial finetune, layers: ", self.args.finetune_partial_layers)
            #print("partial finetune, layers: ", 40)
            for count, (name, param) in enumerate(self.network.named_parameters()):
                #if count > total_params - 40: #int(self.args.finetune_partial_layers) + 1:  
                if 'char' not in name:
    ...
    @staticmethod
    def load_finetune(filename, new_args=None, normalize=True):
        logger.info('Loading model %s' % filename)
        saved_params = torch.load(
            filename, map_location=lambda storage, loc: storage
        )
