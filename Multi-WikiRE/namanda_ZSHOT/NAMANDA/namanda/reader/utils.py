...
from .data import Dictionary, CharDictionary, CombinedDictionary
...
def load_data(args, filename, skip_no_answer=False, id_type_q=True):
    """Load examples from preprocessed file.
    One example per line, JSON encoded. remove sents > 128"""
    # Load JSON lines
    with open(filename, encoding='utf-8') as f:
        examples = [json.loads(line) for line in f]

    # !HACK! don't load examples which have doc or q of length > bert_seq_len !HACK! #
    remove_counter = 0
    new_examples = []
    for ex in examples:
        if len(ex['document']) > 128 or len(ex['question']) > 128:
            remove_counter += 1
        else:
            new_examples.append(ex)
    logger.info('Removed {} examples from the dataset'.format(remove_counter))
...
def load_text(filename):
    """Load the paragraphs only. Store as qid -> text."""
    # Load JSON file
    with open(filename) as f:
...
def load_answers(filename):
    """Load the answers. Store as qid -> [answers]."""
    # Load JSON file
    with open(filename) as f:
        examples = json.load(f)['data']
    ...
    for article in examples:
        for paragraph in article['paragraphs']:
            for qa in paragraph['qas']:
                ...
                ans[qa['id']] = list(map(lambda x: x['text'], qa['answer']))
...
def build_word_and_char_dict(args, examples):
    """Return both word and character dictionary from question and document words in
    provided examples.
    """
    word_dict = Dictionary()
